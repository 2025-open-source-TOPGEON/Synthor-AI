...
class KoreanFullNameExtractor(ConstraintExtractor):
    ...
    def extract(self, text: str) -> dict:
        """
        한국어 전체이름(korean_full_name)에서 성(lastName) 추출
        """
        ...
        primary_ko = [
            ...
            r'성이\s*(' + self._H15 + r')(?=(?:으로|로)\s*(?:끝나는|시작(?:하는)?|되어\s*있는))',
            r'성씨가\s*(' + self._H15 + r')(?=(?:으로|로)\s*(?:끝나는|시작(?:하는)?|되어\s*있는))',
        ]
        for p in primary_ko:
            m = re.search(p, t)
            if m:
        ...
        noun_patterns = [
            r'(?<![가-힣])(' + self._H15 + r')\s*성(?:씨)?\s*(?=(?:을|를|은|는)(?:\s|$))',
            r'(?<![가-힣])성(?:씨)?\s*[:=]\s*(' + self._H15 + r')(?=\s|$)',
            r'(?<!성)(' + self._H15 + r')씨\s*(?:(?:이름|성명|풀네임|전체\s*이름))(?=\s|$)',
            r'(?<!성)(' + self._H15 + r')씨\s*(?=(?:만|면)(?:\s|$))',
            r'(?<![가-힣])(' + self._H15 + r')씨(?=\s|$)',
            r'(' + self._H15 + r')\s*성(?:씨)?\s*(?:을|이)?\s*(?:가진|가지고|쓰는)\b',
        ]
